...
s_no_extra_traits! {
    ...
    struct __c_anonymous__si_fields__sigchld {
        ...
        si_uid: crate::uid_t,
        si_status: c_int,
        // [NOTE]: This is `__sigchld_clock_t` upstream when compiling an x86
        // program to be run in x86_64 compatibility mode. The former is an
        // alias to the latter but forces a 4-byte alignment requirement. That
        // type of compilation is, at the time of writing, not possible in Rust.
        // Thus, using the `linux/gnu/b32/mod.rs`'s definition for `clock_t`
        // should do just fine; It's an `i32` there.
